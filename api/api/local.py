...
df = pd.read_csv('trees_results_topics.tsv', sep='\t')
df['aspects'] = df['aspects'].dropna().apply(dec)
df['sentiment'] = df['sentiment'].dropna().apply(dec)
df['topics'] = df['topics'].dropna().apply(dec)
...
b = BPEmb(lang="ru", vs=100000, dim=200, cache_dir=Path('./bemp'))
morph = MorphAnalyzer()
        ...
        for a, s in zip(aa, ss):
            if a[1::2].replace(' ', '') != '':
                ...
                try:
                    ...
                    aspects_vecs.append(
                        b.embed(ctext).mean(0) if ctext else np.array([0.] * 200)
                    )
                    aspects_data[a] = {
                        'cnt': 1,
                        'Positive': 0,
...
aspect_phrases = phrases(df['aspects'], df['sentiment'])
df['cluster'] = df['aspects'].apply(
    lambda x: sum([
            list(filter(
...
def highlight(row):
    ...
    aspects = row['aspects']
    for a in aspects:
        text = text.replace(a, f'<span class="tag is-rounded is-primary is-outlined">{a}</span>')
    return text
